...
block_list = full_block_list[:num_blocks]
...
def get_next_locations(start_loc: Location) -> list[Location]:
    result = []
...
def get_costs_for_next_locations(start_loc: Location) -> list[Location]:
    next_locations = get_next_locations(start_loc)
    result = []
...
while len(locations_to_be_checked) > 0:
    ...
    next_locations = get_costs_for_next_locations(current_loc)
    locations_to_be_checked.extend(next_locations)

pprint(location_cost)
print(location_cost[(map_height - 1, map_width - 1)])